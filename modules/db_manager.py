...
class DatabaseManager:
        ...
        with self.lock:
            with self.get_connection() as conn:
                ...
                for col_sql in [
                    ...
                    "ALTER TABLE predictions ADD COLUMN side TEXT DEFAULT 'BUY'"
                ]:
                    try:
    ...
    def update_balance(self, amount):
        with self.lock:
            with self.get_connection() as conn:
                ...
                # Internal call to get_balance would deadlock if we didn't use RLock or separate connection.
                # Ideally calculate outside. But here we just read.
                # Since get_balance uses 'with self.lock', we need RLock or just read directly here.
                # Let's read directly here to avoid deadlock if Lock is not RLock.
                res = conn.execute(text('SELECT balance FROM account WHERE id = 1'))
                row = res.fetchone()
    ...
    def get_portfolio(self):
        # Read only, usually doesn't need lock for SQLite if WAL enabled, but safe to include
        # Pandas read_sql might open its own connection? 
        # Yes, passing engine.
        # But we can't easily lock the engine's internal pool usage from here without just wrapping the call.
        with self.lock:
    ...
    def log_prediction(self, ticker, predicted, actual, confidence, strategy, side="BUY"):
        with self.lock:
            with self.get_connection() as conn:
                conn.execute(text('''
                    INSERT INTO predictions (ticker, timestamp, predicted_price, actual_price, confidence, strategy, side)
                    VALUES (:t, :ts, :p, :a, :c, :s, :side)
                '''), {"t": ticker, "ts": datetime.now(), "p": predicted, "a": actual, "c": confidence, "s": strategy, "side": side})
                conn.commit()
